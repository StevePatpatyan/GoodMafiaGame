...
async def create(ctx):
    ...
    if user in users:
        await ctx.author.send(
            "You are already in an active session, so you cannot create another."
        )
        return
    await ctx.author.send(
        "Please type a session password. If you don't want a password, type a single character."
    )
    ...
    await ctx.author.send(
        "Session successfully created. You and others in the session will be alerted when the lobby is full or when there are enough players to start."
    )
    await ctx.channel.send(
        ctx.author.mention
        + ' has successfully started a mafia session! Type "^join" to join.'
    )
...
async def join(ctx):
    ...
    if user in "".join(users).split(","):
        await ctx.author.send(
            "You are already in an active session, so you cannot join another."
        )
        return
    listOfSessions = ""
    if len(users) <= 1:
    ...
    await ctx.author.send(
        "Select an active session to join by typing the number of the session."
    )
    ...
    if passwordResponse == "":
        ...
        await ctx.channel.send(
            ...
            + str(response)
            + " (<@"
            + str(int(users[response - 1].split(",")[0]))
            + ">'s lobby)"
        )
        return
...
async def play(ctx):
    ...
    await ctx.channel.send(
        usersInSession + " <@" + user + "> has started the Mafia Game!"
    )
    #################################################################################
    usersInSession = users[hosts.index(user)].split(",")
    ...
    while len(players) > 2:
        if len(players) > 3:
            ...
            while True:
                ...
                def check3(m):
                    return m.author == mafia and isinstance(
                ...
                try:
                    ...
                    await mafia.send(
                        "That is not an account. Try typing the exact username of the player."
                    )
                    continue
                victim = victim.id
            ...
            while True:
                ...
                def check4(m):
                    return m.author == doctor and isinstance(
                        m.channel, discord.DMChannel
                    )
            ...
            while True:
                ...
                def check5(m):
                    return m.author == detective and isinstance(
                        m.channel, discord.DMChannel
                    )
            ...
            chosenExpo = expos[randint(0, len(expos) - 1)].replace(
                "[VIC]", "<@" + str(victim) + ">"
            )
            chosenDeath = deaths[randint(0, len(deaths) - 1)].replace(
                "[VIC]", "<@" + str(victim) + ">"
            )
            chosenRevival = revivals[randint(0, len(revivals) - 1)].replace(
                "[VIC]", "<@" + str(victim) + ">"
            )
            ...
            if patient == victim:
                ...
                players = {
                    role: player for role, player in players.items() if player != victim
                }
        ...
        for player in players.values():
            ...
            voteCount[player] = 0
            await ctx.channel.send(
                "All in favor of voting for "
                "<@"
                + str(player)
                + ">"
                + " say 'aye' and 'no' to take back your vote. 20 seconds to vote"
            )
            while time() <= timeout:
                try:
                    ...
                    continue
                if (
                    voteMsg.content.lower() == "aye"
                    and whoVotedFor[voteMsg.author.id] == 0
                ):
                    if secretDavidSetting and player:
                        await ctx.channel.send(
                            str(voteMsg.author.id)
                            + "<@"
                            + str(voteMsg.author.id)
                            + ">"
                            + str(voteMsg.author.id)
                    ...
                    await ctx.channel.send(
                        "<@" + str(voteMsg.author.id) + ">" + " You already voted."
                    )
                if (
                    voteMsg.content.lower() == "no"
                    and whoVotedFor[voteMsg.author.id] == 0
                ):
                    await ctx.channel.send(
                        "<@" + str(voteMsg.author.id) + ">" + " You haven't voted yet."
                    )
                elif (
                    voteMsg.content.lower() == "no"
                    and whoVotedFor[voteMsg.author.id] != 0
                    ...
                    await ctx.channel.send(
                        ...
                        + str(voteMsg.author.id)
                        + ">"
                        + " You took away your vote."
                    )
        if list(voteCount.values()).count(list(voteCount.values())[0]) == len(
            list(voteCount.values())
        ):
            await ctx.channel.send("Vote tied evenly... Nobody was eliminated.")
            sleep(2)
        else:
            subPlayers = [
                list(players.values())[x] for x in range(len(players.values()))
            ]
            for vote in voteCount.values():
                tiebreakers = 0
                if list(voteCount.values()).count(vote) > 1 and vote > max(
                    list(voteCount.values())
                ):
                    tiebreakers = vote
            if tiebreakers != 0:
                subPlayers = [
                    player
                    for player in players.values()
                    if voteCount[player] == tiebreakers
                ]
                ...
                for player in subPlayers:
                    ...
                    while time() <= timeout:
                        ...
                        if (
                            voteMsg.content.lower() == "no"
                            and whoVotedFor[voteMsg.author.id] != player
                        ):
                            await ctx.channel.send(
                                "<@"
                                + str(voteMsg.author.id)
                                + "> You haven't voted yet or you already voted for someone else."
                            )
                        elif (
                            voteMsg.content.lower() == "no"
                            and whoVotedFor[voteMsg.author.id] != 0
                ...
                if list(voteCount.values()).count(list(voteCount.values())[0]) == len(
                    ...
                    players = {
                        ...
                        if player
                        != subPlayers[voteCount.values().index(max(voteCount.values()))]
                    }
                    await ctx.channel.send(
                        str(
                            subPlayers[
                                list(voteCount.values()).index(
                                    max(list(voteCount.values()))
                                )
                            ]
                        )
                        + "<@"
                ...
                players = {
                    ...
                    if player
                    != subPlayers[
                        list(voteCount.values()).index(max(list(voteCount.values())))
                    ]
                }
                await ctx.channel.send(
                    str(
        ...
        if "mafia" not in players:
            ...
            return
    await ctx.channel.send(
        "Game over. The mafia was " + +"<@" + str(players["mafia"]) + ">"
    )
    ...
    for x in range(len(users)):
        ...
        if str(ctx.author.id) in group[0]:
            ...
            await ctx.channel.send(
                ...
                + str(ctx.author.id)
                + ">"
                + " you successfully ended your session."
            )
            return
    await ctx.channel.send(
        "<@" + str(ctx.author.id) + ">" + " you are not the host of a session."
    )
...
async def cosmetics(ctx):
    ...
    for equip in equips:
        if (
            equip not in userCosmetics or userCosmetics[equip] == False
        ) and equip != "off":
            await ctx.author.send(
                "You do not have the '" + equip + '" equip boss. Equip cancelled.'
            )
            return
        if equip == "off":
